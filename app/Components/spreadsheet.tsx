...
export default function Spreadsheet({
  ...
  const [savingId, setSavingId] = useState<string | null>(null); // tracks in-flight save
  ...
  async function saveRow(row: Transaction) {
    ...
    // Guard: block re-entrant saves for the same row (double-tap / double-click)
    ...
    try {
      ...
      if (isNew) {
        const { id, ...payload } = row; // never send the temp id as a field
        const docId = await addTransaction(uid, payload);
        setRows(rows.map((r) => (r.id === row.id ? { ...row, id: docId } : r)));
      } else {
        const { id, ...patch } = row; // strip id so it's never written as a field
        await updateTransaction(uid, row.id, patch);
...
//   const [editingId, setEditingId] = useState<string | null>(null);
//   const [errorMsg, setErrorMsg] = useState<string | null>(null);
//   const [savingId, setSavingId] = useState<string | null>(null); // NEW: track in-flight save
...
//         const { id, ...payload } = row; // never send the temp id as a field
//         const docId = await addTransaction(uid, payload);
//         setRows((prev) => prev.map((r) => (r.id === row.id ? { ...row, id: docId } : r)));
//       } else {
//         const { id, ...patch } = row; // strip id so it's never written as a field
//         await updateTransaction(uid, row.id, patch);
//         // keep local state in sync explicitly, rather than relying on a listener
...
//   // ...rest of component (confirmDeleteRow, exportCSV, JSX) unchanged,
